...
export function applyPoisonous(state: GameState, event: TagChange): GameState {
  if (event.tag !== 'POISONOUS') return state;
  ...
  const poisonousOn = event.value === '1';
      ...
      player: {
        ...
        board: {
          ...
          minions: state.player.board.minions.map((m) =>
            m.entityId === entityId ? { ...m, poisonous: poisonousOn } : m,
          ),
        },
      },
    };
