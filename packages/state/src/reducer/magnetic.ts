...
export function applyMagnetic(state: GameState, event: TagChange): GameState {
  if (event.tag !== 'MAGNETIC') return state;
  ...
  const magneticOn = event.value === '1';
      ...
      player: {
        ...
        board: {
          ...
          minions: state.player.board.minions.map((m) =>
            m.entityId === entityId ? { ...m, magnetic: magneticOn } : m,
          ),
        },
      },
    };
