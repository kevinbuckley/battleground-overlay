...
export function applyExhausted(state: GameState, event: TagChange): GameState {
  if (event.tag !== 'EXHAUSTED') return state;
  ...
  const exhaustedOn = event.value === '1';
      ...
      player: {
        ...
        board: {
          ...
          minions: state.player.board.minions.map((m) =>
            m.entityId === entityId ? { ...m, exhausted: exhaustedOn } : m,
          ),
        },
      },
    };
