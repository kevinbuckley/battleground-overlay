...
export function applyWindfury(state: GameState, event: TagChange): GameState {
  if (event.tag !== 'WINDFURY') return state;
  ...
  const windfuryOn = event.value === '1';
      ...
      player: {
        ...
        board: {
          ...
          minions: state.player.board.minions.map((m) =>
            m.entityId === entityId ? { ...m, windfury: windfuryOn } : m,
          ),
        },
      },
    };
