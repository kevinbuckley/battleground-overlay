...
export function applyLifesteal(state: GameState, event: TagChange): GameState {
  if (event.tag !== 'LIFESTEAL') return state;
  ...
  const lifestealOn = event.value === '1';
      ...
      player: {
        ...
        board: {
          ...
          minions: state.player.board.minions.map((m) =>
            m.entityId === entityId ? { ...m, lifesteal: lifestealOn } : m,
          ),
        },
      },
    };
