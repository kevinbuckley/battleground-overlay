...
export function applyImmune(state: GameState, event: TagChange): GameState {
  if (event.tag !== 'IMMUNE') return state;
      ...
      player: {
        ...
        board: {
          ...
          minions: state.player.board.minions.map((m) =>
            m.entityId === entityId ? { ...m, immune: immuneOn } : m,
          ),
        },
      },
    };
