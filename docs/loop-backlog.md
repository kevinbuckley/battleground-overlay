...
- [ ] [S] `parseLine` empty input — in `packages/log-parser/src/index.test.ts`, add 1 test: `parseLine('')` returns `null`; `parseLine('   ')` returns `null` — `packages/log-parser/src/index.test.ts` only
