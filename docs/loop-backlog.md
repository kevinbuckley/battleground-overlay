...
- [ ] [S] `ipcBridge` stop test: in `apps/overlay/src/ipcBridge.test.ts`, add 1 test: start bridge, record send count after first tick (wait 300ms), call `stopBridge()`, wait another 300ms, assert send count is unchanged — `apps/overlay/src/ipcBridge.test.ts` update
- [ ] [S] `settings` save+load roundtrip: in `apps/overlay/src/settings.test.ts`, add 2 tests: (1) save `defaultSettings()` to a tmp file, load from same path, assert all fields equal; (2) save with `opacity: 0.5`, load, assert `opacity === 0.5` — `apps/overlay/src/settings.test.ts` update
- [ ] [S] `getCardName` by string id: in `packages/card-data/src/indexes.ts`, add `getCardName(cardId: string): string` — lazy `byId: Map<string, Card>` built once from `loadCards()`, returns `card.name` if found, returns `cardId` unchanged if not; export from `packages/card-data/src/index.ts`; 3 tests: known id returns name, unknown id returns the id string, empty string returns empty string — `packages/card-data/src/indexes.ts` update + test
- [ ] [S] `getCardsByTribe` filter: in `packages/card-data/src/indexes.ts`, add `getCardsByTribe(tribe: string): Card[]` — filters `loadCards()` results to cards whose `race` field (string) matches tribe case-insensitively; export from card-data index; 3 tests: `'MURLOC'` returns non-empty array, `'INVALID_TRIBE_XYZ'` returns [], all returned cards have matching race — `packages/card-data/src/indexes.ts` update + test
- [ ] [S] `damageWidget` zero guard: in `apps/overlay/src/damageWidget.ts`, verify `computeForecast` returns `{ minDmg: 0, maxDmg: 0, winPct: 0 }` when `scoreResult` has all zeros (no NaN); add 2 tests: zero scoreResult returns all-zero forecast, negative avgHpDelta returns non-negative minDmg (clamp to 0) — `apps/overlay/src/damageWidget.test.ts` update
