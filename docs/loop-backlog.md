...
- [ ] [S] `getCardName` by card string id: in `packages/card-data/src/indexes.ts`, add `getCardName(cardId: string): string` — builds a lazy `byId: Map<string, Card>` on first call using `loadCards()`, returns `card.name` if found, returns `cardId` unchanged if not; export from card-data `index.ts`; 3 tests: known id returns name, unknown id returns input, empty string returns empty string — `packages/card-data/src/indexes.ts` update + test
- [ ] [S] `getCardsByTribe` filter: in `packages/card-data/src/indexes.ts`, add `getCardsByTribe(tribe: string): Card[]` — filters `loadCards()` to cards whose `race` matches the tribe string (case-insensitive); no caching needed; 3 tests: `'MURLOC'` returns non-empty, `'UNKNOWN_TRIBE_XYZ'` returns [], result only contains cards with matching race — `packages/card-data/src/indexes.ts` update + test
- [ ] [S] `damageWidget` zero-minion guard: in `apps/overlay/src/damageWidget.ts`, verify `computeForecast` handles the case where `playerTier` is 0 or `scoreResult.avgHpDelta` is 0 without NaN; add 2 tests: scoreResult with all zeros returns `{ minDmg: 0, maxDmg: 0, winPct: 0 }`, negative avgHpDelta returns non-negative minDmg — `apps/overlay/src/damageWidget.test.ts` update
...
- [ ] [S] `ipcBridge` stop function: in `apps/overlay/src/ipcBridge.ts`, verify `stopBridge()` clears the polling interval so no more IPC pushes happen after it's called; add 1 test: start bridge, call stopBridge after first tick, wait 300ms, assert send count unchanged — `apps/overlay/src/ipcBridge.test.ts` update
- [ ] [S] `settings` save roundtrip: in `apps/overlay/src/settings.ts`, verify `saveSettings` + `loadSettings` roundtrip preserves all fields; add 2 tests: save default settings to tmp path, load from same path, assert all fields equal; save modified opacity 0.5, load, assert opacity 0.5 — `apps/overlay/src/settings.test.ts` update
- [ ] [S] `parseLine` ShowEntity: in `packages/log-parser/src/parseLine.ts`, verify or add parsing of `SHOW_ENTITY` lines (format: `SHOW_ENTITY - Updating Entity=... CardID=...`); returns `{ kind: 'ShowEntity', entityId, cardId }` or null if not matching; 3 tests: valid ShowEntity line, unknown entity, non-ShowEntity returns null — `packages/log-parser/src/parseLine.ts` update + test
