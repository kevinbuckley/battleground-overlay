...
- [ ] [S] `rerollScore` low-gold test — in `packages/advisor/src/heuristics/rerollScore.test.ts`, add 1 test: state with `gold=0` returns `0`; state with `gold=10` and shop full returns a value ≤ 1 — `packages/advisor/src/heuristics/rerollScore.test.ts` only
- [ ] [S] `freezeScore` already-frozen short-circuit — in `packages/advisor/src/heuristics/freezeScore.test.ts`, add 1 test: state where `state.player.shop.frozen = true` returns `0` (no reason to freeze again) — `packages/advisor/src/heuristics/freezeScore.test.ts` only
