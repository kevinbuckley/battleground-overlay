...
- [ ] [S] `minionsOnBoard` / `opponentMinionsOnBoard` helpers: in `packages/shared/src/utils.ts`, add `minionsOnBoard(state: GameState): number` returning `state.player.board.minions.length` and `opponentMinionsOnBoard(state: GameState, index: number): number` returning opponent board size or 0 if out of range; export both; 4 tests: empty=0, 3 minions=3, opponent count, out-of-range=0 — `packages/shared/src/utils.ts` update + test
