...
- [ ] [S] `recommend` sell integration: in `packages/advisor/src/recommend.ts`, after buy candidates, also score sell candidates via `enumerateSellCandidates` + `scoreBuysWithSim`-style scoring, merge into results and re-sort by score; 2 tests: board with weak minion ranks sell above freeze, empty board no sell candidates — `packages/advisor/src/recommend.ts` update + test
