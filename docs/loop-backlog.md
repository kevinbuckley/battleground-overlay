...
- [ ] [S] `scoreBuysWithSim` n=1 smoke test: in `packages/advisor/src/budgetScorer.test.ts`, add a test that calls `scoreBuysWithSim` with `n=1` and `budgetMs=5000` on a state with 1 shop minion and verifies the result has `score >= 0` and `action.type === 'Buy'`; confirms real sim path executes without error — `packages/advisor/src/budgetScorer.test.ts` update
