...
- [ ] [S] Coordinator logs parsed events — in `apps/overlay/src/coordinator.ts`, inside the wrapped `pipeline.onEvent`, before calling `originalOnEvent(event)`, call `(opts?.logFn ?? appendSessionEvent)('event', { kind: event.kind })`; 2 tests in `coordinator.test.ts`: (1) firing a TAG_CHANGE event causes one `event` log entry with `kind: 'TAG_CHANGE'`, (2) firing 3 events causes 3 `event` entries — `apps/overlay/src/coordinator.ts` + `apps/overlay/src/coordinator.test.ts`
