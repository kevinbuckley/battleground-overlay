...
- [ ] [S] `stream.ts` add missing parsers — `parseSingleLine` in `packages/log-parser/src/stream.ts` misses `parseZoneChangeList` and `parseShowEntity`; add them to the `??` chain; 2 tests in stream.test.ts: a `ZONE_CHANGE_LIST` line is picked up as `{ kind: 'ZONE_CHANGE_LIST' }`, a `SHOW_ENTITY - Updating Entity=5 CardID=CS2_168` line is picked up as `{ kind: 'SHOW_ENTITY' }` — `packages/log-parser/src/stream.ts` + test
