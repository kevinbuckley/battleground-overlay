...
- [ ] [S] `registerIpcHandlers` extraction — extract the 6 `ipcMain.handle(...)` calls from `apps/overlay/src/main.ts` into a new exported function `registerIpcHandlers(ipc: { handle(channel: string, cb: (...args: unknown[]) => unknown): void }, deps: { settings: { loadSettings: typeof loadSettings }, panels: { setAdvice, setExplanation, setBoardPanel, setOpponentPanel, setInteractive } })` in `apps/overlay/src/ipcHandlers.ts`; 3 tests: a fake `ipc` whose `handle` calls are recorded shows exactly the 6 channels `'set-interactive'`, `'set-advice'`, `'set-explanation'`, `'set-board-panel'`, `'set-opponent-panel'`, `'settings:apply'`; invoking the `set-interactive` handler with `true` calls `setInteractive(true)`; invoking the `set-advice` handler with a recommendation object calls `setAdvice` once — `apps/overlay/src/ipcHandlers.ts` + `apps/overlay/src/ipcHandlers.test.ts`
