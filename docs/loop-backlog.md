...
- [ ] [S] `loadSettings` empty-string path returns defaults — modify `loadSettings(path)` in `apps/overlay/src/settings.ts` so that when `path === ''` it returns `defaultSettings()` without throwing; 1 test in `settings.test.ts`: `loadSettings('')` deep-equals `defaultSettings()` — `apps/overlay/src/settings.ts` + `apps/overlay/src/settings.test.ts`
