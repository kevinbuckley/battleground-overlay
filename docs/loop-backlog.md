...
- [ ] [S] `loadSettings` invalid JSON fallback — in `apps/overlay/src/settings.test.ts`, add 1 test: write a file with `{invalid json}` to a temp path, call `loadSettings(tmpPath)`, assert it returns `defaultSettings()` without throwing — `apps/overlay/src/settings.test.ts` only
