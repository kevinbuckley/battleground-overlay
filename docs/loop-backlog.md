...
- [ ] [S] `recommend` Reposition rec test — in `packages/advisor/src/recommend.test.ts`, add 1 test: state with 3-minion board and 1 opponent with different minions, call `recommend(state)` with `n=5` sims; assert at least one rec is returned (hill-climb runs and doesn't throw) — `packages/advisor/src/recommend.test.ts` only
