...
- [ ] [S] `parseShowEntity` in log parser: in `packages/log-parser/src/parseLine.ts`, add parsing for `SHOW_ENTITY - Updating Entity=N CardID=X` lines alongside existing parsers; return `{ kind: 'ShowEntity', entityId: N, cardId: 'X' }` when matched, existing null otherwise; 3 tests: valid line returns correct shape, line without CardID returns null, unrelated line returns null — `packages/log-parser/src/parseLine.ts` update + test
