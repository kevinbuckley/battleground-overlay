...
- [ ] [S] `clamp` boundary tests — in `packages/shared/src/utils.test.ts`, add 3 tests: `clamp(5,5,10) === 5`, `clamp(10,5,10) === 10`, `clamp(NaN,1,3)` returns NaN or 1 (assert whichever matches implementation) — `packages/shared/src/utils.test.ts` only
