...
- [ ] [S] `setAnchorStatus` / `getAnchorStatus` — add to `apps/overlay/src/anchor.ts` module-level state with exported functions `setAnchorStatus(s: 'waiting'|'anchored'|'failed'): void` and `getAnchorStatus(): 'waiting'|'anchored'|'failed'`; default 'waiting'; 3 tests: initial value is `'waiting'`; after `setAnchorStatus('anchored')` returns `'anchored'`; after toggle to `'failed'` then back to `'anchored'` returns `'anchored'` — `apps/overlay/src/anchor.ts` + `apps/overlay/src/anchor.test.ts`
