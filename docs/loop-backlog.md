...
- [ ] [S] `stream.ts` close stops watcher — in `packages/log-parser/src/stream.test.ts`, add 1 test: open stream on empty temp file, call `handle.close()`, append a TAG_CHANGE line, wait 400ms, assert events array is still empty — `packages/log-parser/src/stream.test.ts` only
