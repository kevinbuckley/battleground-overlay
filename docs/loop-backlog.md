...
- [ ] [S] Renderer confidence percent — in `apps/overlay/src/renderer.ts`, when `<span id="advice-confidence">` exists, set textContent to `Math.round(top.confidence * 100) + '%'`; add `<span id="advice-confidence"></span>` to `renderer.html`; 2 tests: top confidence `0.84` → `"84%"`; top confidence `0` → `"0%"` — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.html` + `apps/overlay/src/renderer.test.ts`
