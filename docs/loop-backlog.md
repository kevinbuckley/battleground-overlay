...
- [ ] [S] `ipcBridge` stop test: in `apps/overlay/src/ipcBridge.test.ts`, add 1 test: start bridge with mock window/getState/getRecs, wait 300ms for first tick, record send count, call `stopBridge()`, wait 300ms more, assert send count unchanged — `apps/overlay/src/ipcBridge.test.ts` update
- [ ] [S] `settings` save+load roundtrip: in `apps/overlay/src/settings.test.ts`, add 2 tests using `/tmp` paths: (1) save `defaultSettings()`, reload with `loadSettings`, assert all fields equal; (2) save with `opacity: 0.5`, reload, assert `opacity === 0.5` — `apps/overlay/src/settings.test.ts` update
- [ ] [S] `getCardName` by string id: in `packages/card-data/src/indexes.ts`, add `getCardName(cardId: string): string` — lazy `byId: Map<string, Card>` from `loadCards()`, returns `card.name` if found, `cardId` unchanged if not; export from `packages/card-data/src/index.ts`; 3 tests: known id returns name, unknown returns id string, empty string returns empty — `packages/card-data/src/indexes.ts` update + test
- [ ] [S] `getCardsByTribe` filter: in `packages/card-data/src/indexes.ts`, add `getCardsByTribe(tribe: string): Card[]` filtering `loadCards()` to cards whose `race` matches tribe case-insensitively; export from card-data index; 3 tests: `'MURLOC'` returns non-empty, `'INVALID_XYZ'` returns [], all returned cards have matching race — `packages/card-data/src/indexes.ts` update + test
- [ ] [S] `recommend` freeze/reroll heuristic: in `packages/advisor/src/recommend.ts`, when shopping phase and `state.player.shop.frozen === false` and shop has ≥2 minions, add a Freeze rec with score 0.4; when gold ≥ rollCost, add a Reroll rec with score 0.2; sort into results; 3 tests: freeze appears when shop unfrozen with ≥2 minions, reroll appears when can afford, neither appears in combat phase — `packages/advisor/src/recommend.ts` update + test
- [ ] [S] `applyGoldSpent` tracker: handles `TAG_CHANGE tag=RESOURCES_USED` on player controller → sets `state.player.goldSpentThisTurn: number` (add field to PlayerState); wire into reducer; 4 tests (initial=0, updates on spend, no-op on opponent, tracks multiple spends) — `packages/state/src/reducer/goldSpent.ts` + test
