...
- [ ] [S] `formatRecommendation` in shared: in `packages/shared/src/recommendation.ts`, add `formatRecommendation(rec: Recommendation): string` returning a readable string — `"Buy <action.cardId> (score: 0.72)"` for Buy, `"Sell position <action.boardIndex> (score: 0.45)"` for Sell, `"Freeze shop (score: 0.40)"` for Freeze, `"Reroll (score: 0.30)"` for Reroll; 4 tests: one per action type — `packages/shared/src/recommendation.ts` update + test
