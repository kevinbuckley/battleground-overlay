...
- [ ] [S] `isBattlegroundsPool` golden card test: in `packages/card-data/src/isBattlegroundsPool.test.ts`, add 2 tests: a golden variant (cardId ending in `_golden`) of a known pool card still returns true via base-id lookup, and a non-BG card returns false — `packages/card-data/src/isBattlegroundsPool.test.ts` update
...
- [ ] [S] `coordinator` stop test: in `apps/overlay/src/coordinator.test.ts`, add a test that calls `startCoordinator(mockWin)` then `.stop()`, then dispatches an event to the pipeline, and asserts `win.webContents.send` is NOT called after stop (bridge is down); also assert `stop()` is idempotent (calling twice doesn't throw) — `apps/overlay/src/coordinator.test.ts` update
- [ ] [S] `withBudget` time-cap test: in `packages/advisor/src/withBudget.test.ts`, add a test that calls `scoreBuysWithBudget(state, { budgetMs: 100 })` and asserts it returns within 200ms (use `performance.now()` before/after); also assert the returned array has `score >= 0` for each entry — `packages/advisor/src/withBudget.test.ts` update
- [ ] [S] `buildPrompt` recommendation test: in `packages/llm/src/buildPrompt.test.ts`, add a test that calls `buildPrompt(recommendation, state)` with a Buy recommendation and verifies the returned string contains the card id, the action type `'Buy'`, and a score digit — `packages/llm/src/buildPrompt.test.ts` update
- [ ] [S] `LLM cache` deduplication test: in `packages/llm/src/cache.test.ts`, add a test creating two identical `initialState()` states, calling `ExplanationCache.get(state1)` (null), setting via `ExplanationCache.set(state1, "text")`, then calling `ExplanationCache.get(state2)` and asserting it returns `"text"` — same hash → cache hit — `packages/llm/src/cache.test.ts` update
- [ ] [S] `reviewSession` markdown output: in `scripts/review-session.test.ts`, add a test calling `reviewSession(sessionEvents)` with 2 turn-snapshots and 1 recommendation event, and asserting the output string contains `## Turn`, a score value, and the action type string — `scripts/review-session.test.ts` update
- [ ] [S] `scrubber` edge cases: in `apps/replay/src/scrubber.test.ts`, add 2 tests: (1) calling `stepBackward()` at tick 0 does not throw and returns tick 0, (2) calling `stepForward()` past the last event returns the final state without throwing — `apps/replay/src/scrubber.test.ts` update
