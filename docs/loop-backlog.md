...
- [ ] [S] Renderer `onBoard` in OverlayBridge — add `onBoard(cb: (b: unknown) => void): void` to the `OverlayBridge` interface in `apps/overlay/src/renderer.ts`; in `initRenderer` call `bridge.onBoard(...)` and update element `#board-count` textContent with `Minions: N` (where N = `(b as {minions: unknown[]}).minions.length`); add `<div id="board-count"></div>` to `apps/overlay/src/renderer.html`; 3 tests: count shows correct N, missing element is no-op, empty minions shows `Minions: 0` — `renderer.ts` + `renderer.html` + `renderer.test.ts`
