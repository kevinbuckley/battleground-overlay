...
- [ ] [S] `applyShopSize` — handles `TAG_CHANGE tag=NUM_MINIONS_IN_BOB_DECK` on player controller → sets `state.player.shopSize: number` (add field to `PlayerState` in `packages/shared/src/state.ts` and `initialState` in `packages/state/src/initialState.ts`); wire into reducer; 4 tests (initial=3, updates on tavern upgrade, no-op on opponent, correct value persisted) — `packages/state/src/reducer/shopSize.ts` + test
