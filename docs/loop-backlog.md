...
- [ ] [S] Renderer top-3 recs list — in `apps/overlay/src/renderer.ts`, modify the `onRecs` handler so that, when `<ul id="advice-list">` exists in the DOM, it populates up to 3 `<li>` children with `getActionText(rec)`; add `<ul id="advice-list"></ul>` to `renderer.html`; 3 tests in `renderer.test.ts` using a jsdom-style DOM stub: 1 rec → 1 `<li>` with text from `getActionText`; 3 recs → 3 `<li>`; 5 recs → exactly 3 `<li>` (capped) — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.html` + `apps/overlay/src/renderer.test.ts`
