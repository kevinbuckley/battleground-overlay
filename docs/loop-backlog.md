...
- [ ] [S] `main.ts` export `wireLogStream` helper — add exported async function `wireLogStream(onEvent: (e: import('@overlay/log-parser').HsEvent) => void, logBaseDir?: string): Promise<import('@overlay/log-parser').StreamHandle | null>` to `apps/overlay/src/main.ts`; it calls `findActiveLogDir(logBaseDir)` + `join(..., 'Power.log')` + `streamEvents(path, onEvent)` in try/catch returning null on error; call it from `createWindow()` after `startCoordinator`; 3 tests in `main.test.ts`: (1) given a real temp dir with a `Hearthstone_2024-01-01` subdir containing `Power.log` with one TAG_CHANGE line, `wireLogStream(onEvent, tmpBase)` resolves with a handle and `onEvent` was called, (2) non-existent basedir returns null without throwing, (3) `createWindow()` does not throw when log dir is missing (swallows) — `apps/overlay/src/main.ts` + `apps/overlay/src/main.test.ts`
- [ ] [S] `fixtureTest.ts` mismatch throws — in `packages/log-parser/src/fixtureTest.test.ts`, add 2 tests: (1) 2-event file but only 1 expected → throws "count mismatch", (2) matching count but wrong kind at index 0 → throws with "kind" in message — `packages/log-parser/src/fixtureTest.test.ts` only
- [ ] [S] `scripts/check-patch.ts` CI check — create `scripts/check-patch.ts` that imports `patchVersion` from `../packages/card-data/src/patchVersion`; reads `process.argv[2]` as expected version; `console.log`s "OK" and exits 0 on match, `console.error`s "Patch mismatch: pinned=X installed=Y" and exits 1 on mismatch, exits 1 with usage message if arg missing; 3 tests in `scripts/check-patch.test.ts`: match exits 0, mismatch exits 1 with message, no arg exits 1 — `scripts/check-patch.ts` + `scripts/check-patch.test.ts`
- [ ] [S] `parseLine` SHOW_ENTITY + ZONE_CHANGE_LIST roundtrip — in `packages/log-parser/src/index.test.ts`, add 1 test: `parseLine('ZONE_CHANGE_LIST ID=99')` returns `{ kind: 'ZONE_CHANGE_LIST', id: 99 }`; add 1 test: `parseLine('SHOW_ENTITY - Updating Entity=5 CardID=CS2_168')` returns object with `kind: 'SHOW_ENTITY'` and `cardId: 'CS2_168'` — `packages/log-parser/src/index.test.ts` only
- [ ] [S] `Benchmark.compareBenchmarks` degraded winPct — `compareBenchmarks` returns a string; in `packages/sim/src/bench.test.ts`, add 1 test: baseline has `winPct: 0.6`, current has `winPct: 0.4` — assert returned string contains `'↓'`; add 1 test: baseline and current have same `winPct` → returned string contains `'no change'` — `packages/sim/src/bench.test.ts` only
