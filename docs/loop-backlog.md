...
- [ ] [S] `tripleScore` no-board-match test — in `packages/advisor/src/heuristics/triple.test.ts`, add 1 test: shopCard with `cardId='X'` and board with two minions of `cardId='Y'` and `cardId='Z'` returns `0` — `packages/advisor/src/heuristics/triple.test.ts` only
...
- [ ] [S] `getCardName` missing card test — in `packages/card-data/src/indexes.test.ts`, add 1 test: `getCardName('NONEXISTENT_CARD_ID_XYZ')` returns `'NONEXISTENT_CARD_ID_XYZ'` (falls back to the id string, doesn't throw) — `packages/card-data/src/indexes.test.ts` only
...
- [ ] [S] Renderer damage min/max display — in `apps/overlay/src/renderer.ts`, change the `onDamage` handler to set `#damage-forecast` textContent to `Win: <P>% (<min>-<max> dmg)` using `f.minDmg` and `f.maxDmg`; the forecast object already has those fields per `DamageForecast`; 2 tests in `renderer.test.ts`: minDmg=2, maxDmg=5, winPct=0.7 → contains `"Win: 70%"` and `"2-5 dmg"`; missing minDmg falls back to `"?"` or `0` — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.test.ts`
- [ ] [S] Renderer board panel stat hint — in `apps/overlay/src/renderer.ts`, extend `onBoard` to also write the highest-attack minion to a new element `#board-best-attack` as `Best: <attack>/<health>` (use Math.max on minion.attack); add `<div id="board-best-attack"></div>` to renderer.html; 2 tests: 2-minion board with attacks 3,5 → contains `Best: 5/`; empty minions → element textContent is empty — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.html` + `apps/overlay/src/renderer.test.ts`
- [ ] [S] Renderer opponent-eliminated indicator — in `apps/overlay/src/renderer.ts`, extend `onOpponents` to count how many opponents have `eliminated: true` and write `Alive: <N>/<total>` to a new element `#opponent-alive`; add `<div id="opponent-alive"></div>` to renderer.html; 2 tests: 5 opponents with 2 eliminated → `Alive: 3/5`; all eliminated → `Alive: 0/3` — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.html` + `apps/overlay/src/renderer.test.ts`
- [ ] [S] `getActionText` Sell test — in `apps/overlay/src/renderer.test.ts`, add 1 test: `getActionText({ action: {type: 'Sell', boardIndex: 2}, score: 0.5, confidence: 0.5, reason: '' })` returns a string containing `"Sell"` and `"2"` — `apps/overlay/src/renderer.test.ts` only
- [ ] [S] `getActionText` TierUp test — in `apps/overlay/src/renderer.test.ts`, add 1 test: `getActionText({ action: {type: 'TierUp'}, score: 0.6, confidence: 0.6, reason: '' })` returns a string containing `"Tier"` — `apps/overlay/src/renderer.test.ts` only
- [ ] [S] `saveSettings` round-trip — in `apps/overlay/src/settings.test.ts`, add 1 test: write defaultSettings to temp path with `saveSettings`, read back with `loadSettings`, deep-equal the result; use `mkdtempSync`/`rmSync` for cleanup — `apps/overlay/src/settings.test.ts` only
- [ ] [S] `defaultHotkeyConfig` shape test — in `apps/overlay/src/hotkeys.test.ts`, add 1 test: `defaultHotkeyConfig()` returns an object with keys `toggle`, `reload`, `hide` all of `typeof === 'string'` — `apps/overlay/src/hotkeys.test.ts` only
- [ ] [S] `computeDamageForecast` zero-winPct test — in `apps/overlay/src/damageWidget.test.ts`, add 1 test: `computeDamageForecast({ winPct: 0, avgHpDelta: -3 }, 4)` returns an object with `winPct: 0` and `minDmg ≤ maxDmg` (consistency) — `apps/overlay/src/damageWidget.test.ts` only
- [ ] [S] `getHearthstoneBounds` resilience test — in `apps/overlay/src/anchor.test.ts`, add 1 test: calling `getHearthstoneBounds()` 3 times in a row doesn't throw (idempotent) — `apps/overlay/src/anchor.test.ts` only
- [ ] [S] `setInteractive` toggle test — in `apps/overlay/src/overlayState.test.ts`, add 1 test: after `setInteractive(true)` then `setInteractive(false)`, the second call doesn't throw and module state is consistent (verify by checking no exceptions thrown) — `apps/overlay/src/overlayState.test.ts` only
