...
- [ ] [S] `predictOpponentBoard` scale cap test — in `packages/advisor/src/opponentPredictor.test.ts`, add 1 test: at turn 20 (far past cap), `attack` and `health` are at most `1.5 × original` (cap enforced); also add 1 test: turn 4 (boundary) returns unscaled board — `packages/advisor/src/opponentPredictor.test.ts` only
