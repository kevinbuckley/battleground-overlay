...
- [ ] [S] `applyAttackBuff` — handles `TAG_CHANGE tag=ATK` on minion entities in `state.player.board` or opponent boards (matched by entityId) → updates `minion.attack: number`; wire into reducer; 4 tests (updates player board minion, updates opponent board minion, no-op on hero entity, no-op when entityId not on any board) — `packages/state/src/reducer/attackBuff.ts` + test
