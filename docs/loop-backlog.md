...
- [ ] [S] `main.ts` wire `streamEvents` to coordinator — import `{ streamEvents, findActiveLogDir }` from `@overlay/log-parser` and `{ join }` from `node:path`; in `createWindow()`, call `startCoordinator(win)` then `streamEvents(join(findActiveLogDir(), 'Power.log'), coordinator.onEvent)` in a try/catch that swallows ENOENT; 2 tests in `main.test.ts`: mock coordinator receives events from a temp file, missing log dir does not throw — `apps/overlay/src/main.ts` + `apps/overlay/src/main.test.ts`
