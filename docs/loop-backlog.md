...
- [ ] [S] Renderer wire damage display — add `onDamage(cb: (f: unknown) => void): void` to the `OverlayBridge` interface in `apps/overlay/src/renderer.ts`; in `initRenderer`, call `bridge.onDamage((f) => {...})` and update element `#damage-forecast` textContent with `Win: ${pct}%` format; 3 tests: element updated on damage event, missing element is no-op, winPct=0 shows `Win: 0%` — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.test.ts`
