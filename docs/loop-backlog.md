...
- [ ] [S] Preload expose damage + board channels — in `apps/overlay/src/preload.ts`, add `onDamage(cb: (f: unknown) => void)` and `onBoard(cb: (b: unknown) => void)` to `setupPreload`'s `exposeInMainWorld` call (listening on `overlay:damage-update` and `overlay:board-update`); 3 tests: onDamage callback fires with payload, onBoard callback fires with payload, onOpponents is not yet exposed (still only recs+explanation+damage+board after change) — `apps/overlay/src/preload.ts` + `apps/overlay/src/preload.test.ts`
