...
- [ ] [S] `weightedScore` edge cases: in `packages/advisor/src/weightedScore.ts`, verify `weightedWinScore(scoreResult, weights)` handles empty weights array (returns 0) and mismatched lengths (uses min length); add 3 tests: empty weights returns 0, single-weight returns scoreResult.winPct * weight, weight array longer than opponents uses all weights — `packages/advisor/src/weightedScore.test.ts` update
