...
- [ ] [S] Preload: expose `onOpponents` channel — in `apps/overlay/src/preload.ts`, add `onOpponents(cb: (o: unknown[]) => void): void` to the `exposeInMainWorld` call, listening on `overlay:opponents-update`; update the existing test that asserts `bridge.onOpponents` is undefined (change it to assert `onOpponents` is a function and fires with payload); add 1 new passing test for the channel — `apps/overlay/src/preload.ts` + `apps/overlay/src/preload.test.ts`
- [ ] [S] Renderer: `onOpponents` display — add `onOpponents(cb: (o: unknown[]) => void): void` to `OverlayBridge` in `apps/overlay/src/renderer.ts`; in `initRenderer` wire it to update `#opponent-count` textContent with `Opponents: N`; add `<div id="opponent-count"></div>` to `renderer.html`; 3 tests: element updated with opponent count, missing element no-op, 0 opponents shows `Opponents: 0` — `renderer.ts` + `renderer.html` + `renderer.test.ts`
- [ ] [S] `main.ts` settings-apply IPC — add `ipcMain.handle('settings:apply', (_event, path: string) => { const s = loadSettings(path); getOverlayWin()?.setOpacity(s.opacity); getOverlayWin()?.setPosition(s.x, s.y); return true; })` to `apps/overlay/src/main.ts`; import `loadSettings` from `./settings` and `getOverlayWin` from `./overlayState`; 2 tests in `main.test.ts`: handler exists and returns true, non-existent path returns default opacity — `apps/overlay/src/main.ts` + `apps/overlay/src/main.test.ts`
- [ ] [S] `review-session.ts` filterByTurnRange — in `scripts/review-session.ts`, add `filterByTurnRange(entries: SessionEntry[], minTurn: number, maxTurn: number): SessionEntry[]` returning entries where payload has `turn >= minTurn && turn <= maxTurn`; export it; 3 tests: entries in range returned, entries outside excluded, empty input returns [] — `scripts/review-session.ts` + `scripts/review-session.test.ts`
- [ ] [S] `hotkeys` unregisterAll on re-register — in `apps/overlay/src/hotkeys.test.ts`, add 1 test: call `registerHotkeys` twice with the same mock `app`; assert `app.globalShortcut.unregisterAll` was called at least once (cleanup before second registration) — `apps/overlay/src/hotkeys.test.ts` only
- [ ] [S] `formatRecommendation` Reposition test — in `packages/shared/src/recommendation.test.ts`, check if a Reposition test exists; if not, add 1 test: `formatRecommendation({ action: { type: 'Reposition', fromIndex: 1, toIndex: 3 }, score: 0.6, confidence: 0.7, reason: '' })` returns a string containing `'Reposition'`, `'1'`, and `'3'` — `packages/shared/src/recommendation.test.ts` only
