...
- [ ] [S] Renderer per-minion list — in `apps/overlay/src/renderer.ts`, extend `onBoard` to populate `<ul id="board-minions">` with one `<li>` per minion using `formatMinionLine`; add `<ul id="board-minions"></ul>` to `renderer.html`; 2 tests: 2-minion board → 2 `<li>` with stat strings; empty `minions: []` → 0 `<li>` — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.html` + `apps/overlay/src/renderer.test.ts`
