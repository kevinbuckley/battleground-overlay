...
- [ ] [S] `enumerateSellCandidates` in advisor: `packages/advisor/src/candidates.ts` — add `enumerateSellCandidates(state: GameState): { action: { type: 'Sell'; boardIndex: number }; projectedBoard: Board }[]` iterating over `state.player.board.minions`; returns one candidate per minion with that minion removed; 3 tests: empty board returns [], 1 minion returns 1 candidate, 3 minions returns 3 candidates — `packages/advisor/src/candidates.ts` update + test
- [ ] [S] `enumerateFreezeCandidates` in advisor: `packages/advisor/src/candidates.ts` — add `enumerateFreezeCandidates(state: GameState): { action: { type: 'Freeze' }; projectedBoard: Board }[]` returning a single-element array when `state.player.shop.frozen === false`, empty array otherwise (already frozen = no benefit); 3 tests: unfrozen returns 1 candidate, already frozen returns [], action type is 'Freeze' — `packages/advisor/src/candidates.ts` update + test
- [ ] [S] `enumerateRerollCandidates` in advisor: `packages/advisor/src/candidates.ts` — add `enumerateRerollCandidates(state: GameState): { action: { type: 'Reroll' }; projectedBoard: Board }[]` returning a single-element array when `state.player.gold >= state.player.shop.rollCost`, empty array otherwise (can't afford); 3 tests: can afford returns 1 candidate, can't afford returns [], action type is 'Reroll' — `packages/advisor/src/candidates.ts` update + test
- [ ] [S] `enumerateTierUpCandidates` in advisor: `packages/advisor/src/candidates.ts` — add `enumerateTierUpCandidates(state: GameState): { action: { type: 'TierUp' }; projectedBoard: Board }[]` returning a single-element array when `state.player.gold >= state.player.tierUpCost && state.player.tier < 6`, empty array otherwise; 3 tests: affordable tier-3 returns 1 candidate, tier 6 returns [], too-expensive returns [] — `packages/advisor/src/candidates.ts` update + test
