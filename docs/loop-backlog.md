...
- [ ] [S] `hotkeys` unregister-on-quit: in `apps/overlay/src/hotkeys.ts`, verify `registerHotkeys` calls `app.on('will-quit', ...)` to call `globalShortcut.unregisterAll()`; add 1 test: mock app with `on` spy, call `registerHotkeys`, assert `on` called with `'will-quit'` — `apps/overlay/src/hotkeys.test.ts` update
