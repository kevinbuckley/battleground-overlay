...
- [ ] [S] `getCardName(cardId: string): string` in `packages/card-data/src/indexes.ts` — looks up a card by `id` field (not dbfId) and returns its `name`; returns the `cardId` string unchanged if not found; export from card-data index; 4 tests: known id returns name, unknown id returns input, empty string returns empty string, index built lazily — `packages/card-data/src/indexes.ts` update + test
