...
- [ ] [S] `getDefaultSettingsPath()` — add to `apps/overlay/src/settings.ts` a new exported function `getDefaultSettingsPath(homedir = require('node:os').homedir()): string` returning `path.join(homedir, 'Library/Application Support/battleground-overlay', 'overlay-settings.json')`; 2 tests in `settings.test.ts`: returns string ending with `overlay-settings.json`; passing `'/tmp/test'` as homedir returns a string starting with `/tmp/test/` — `apps/overlay/src/settings.ts` + `apps/overlay/src/settings.test.ts`
