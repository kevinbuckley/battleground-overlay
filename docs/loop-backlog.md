...
- [ ] [S] `formatMinionLine` helper — add to `apps/overlay/src/renderer.ts` an exported function `formatMinionLine(m: { attack: number; health: number; cardId: string }): string` returning `\`${m.attack}/${m.health} ${m.cardId}\``; 2 tests: `{attack:3,health:4,cardId:'X'}` returns `"3/4 X"`; `{attack:0,health:1,cardId:''}` returns `"0/1 "` — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.test.ts`
