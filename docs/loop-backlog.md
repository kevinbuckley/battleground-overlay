...
- [ ] [S] `appendSessionEvent` auto-prune on reset — in `packages/shared/src/sessionLog.ts`, modify `resetSession()` to call `pruneOldSessions(50)` after clearing `sessionFile`; 2 tests: after `resetSession()`, if >50 sessions exist only 50 remain; if ≤50 sessions, none deleted — `packages/shared/src/sessionLog.ts` + `packages/shared/src/sessionLog.test.ts`
