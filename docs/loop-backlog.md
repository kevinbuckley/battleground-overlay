...
- [ ] [S] `preload` channel whitelist test: in `apps/overlay/src/preload.test.ts`, add 2 tests: verify the preload bridge only exposes `overlay:state-update`, `overlay:board-update`, `overlay:opponents-update`, `overlay:damage-update` channels (no others), and that `onRecs` and `onExplanation` callbacks are invoked when those channels fire — `apps/overlay/src/preload.test.ts` update
