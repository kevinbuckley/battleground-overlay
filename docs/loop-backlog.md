...
- [ ] [S] `settings` save+load roundtrip: in `apps/overlay/src/settings.test.ts`, add 2 tests using `tmp` paths: (1) save `defaultSettings()`, reload with `loadSettings`, assert all fields equal; (2) save settings with `opacity: 0.5`, reload, assert `opacity === 0.5` — `apps/overlay/src/settings.test.ts` update
- [ ] [S] `getCardName` by string id: in `packages/card-data/src/indexes.ts`, add `getCardName(cardId: string): string` — lazy `byId: Map<string, Card>` from `loadCards()`, returns `card.name` if found, `cardId` unchanged if not; export from `packages/card-data/src/index.ts`; 3 tests: known id returns name, unknown returns id, empty string returns empty — `packages/card-data/src/indexes.ts` update + test
