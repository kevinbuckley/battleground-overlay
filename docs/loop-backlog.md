...
- [ ] [S] `anchorToHearthstoneWithRetry` — add to `apps/overlay/src/anchor.ts` an exported function `async function anchorToHearthstoneWithRetry(win: BrowserWindow, opts?: { maxAttempts?: number; retryMs?: number; offsets?: { x?: number; y?: number } }, anchorFn?: typeof anchorToHearthstone): Promise<boolean>` that retries `anchorFn` up to `maxAttempts` (default 5) with `retryMs` between attempts (default 1000); returns true if any attempt succeeded; 3 tests with stub `anchorFn`: returns true if first attempt returns true; returns false after maxAttempts=1 if attempt returns false; calls anchorFn exactly 3 times when maxAttempts=3 and all return false — `apps/overlay/src/anchor.ts` + `apps/overlay/src/anchor.test.ts`
