...
- [ ] [S] `lobbyWeight` total: in `packages/advisor/src/lobbyWeight.ts`, add `totalLobbyWeight(opponents: OpponentState[]): number` summing all weights from `lobbyWeights(opponents)`; export it; 3 tests: empty returns 0, all equal hp returns sum of all weights, eliminated opponent contributes 0 — `packages/advisor/src/lobbyWeight.ts` update + test
