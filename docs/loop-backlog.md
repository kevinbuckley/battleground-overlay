...
- [ ] [S] Coordinator: call `setBoardPanel` for Reposition rec — in `apps/overlay/src/coordinator.ts`, import `{ setBoardPanel }` from `@overlay/shared`; after `setAdvice(top)`, if `top.action.type === 'Reposition'` call `setBoardPanel({ recommendation: top })`; 2 tests in `coordinator.test.ts`: (1) Reposition rec causes `getBoardPanel().recommendation` to equal the rec, (2) non-Reposition rec does NOT update boardPanel — `apps/overlay/src/coordinator.ts` + test
