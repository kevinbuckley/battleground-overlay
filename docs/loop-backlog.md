...
- [ ] [S] Coordinator logs LLM round-trips — in `apps/overlay/src/coordinator.ts`, wrap the existing `explain(top, state)` call so that on resolve it calls `logFn('llm', { rec: top.action.type, text })` and on reject it calls `logFn('llm-error', { rec: top.action.type })`; 2 tests using a custom `logFn`: (1) when a Buy rec triggers `explain` and resolves with "hello", a `llm` entry with `text: 'hello'` is logged; (2) when `explain` rejects, an `llm-error` entry is logged — `apps/overlay/src/coordinator.ts` + `apps/overlay/src/coordinator.test.ts`
