...
- [ ] [S] `tribeSynergyScore` no-tribe candidate — add 2 tests in `packages/advisor/src/heuristics/tribeSynergy.test.ts`: candidate with empty `tribes: []` against 3-minion board returns `0`; candidate with one tribe matching 3 board minions returns a score ≥ `3 * 0.15 - 0.001` (matches per-minion bonus capped at MAX_SCORE) — `packages/advisor/src/heuristics/tribeSynergy.test.ts` only
