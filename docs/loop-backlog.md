...
- [ ] [S] `applyHealthBuff` — handles `TAG_CHANGE tag=HEALTH` on minion entities in board (distinct from `applyPlayerDeath` which handles HEALTH on player controller) → updates `minion.health: number`; wire into reducer; 4 tests (updates player board minion, updates opponent board minion, no-op on hero/controller entity, no-op when entityId not on any board) — `packages/state/src/reducer/healthBuff.ts` + test
