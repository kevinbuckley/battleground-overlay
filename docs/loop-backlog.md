...
- [ ] [S] Coordinator snapshot logging — in `apps/overlay/src/coordinator.ts`, after calling `originalOnEvent`, if `pipeline.getState().turn` increased compared to the previous turn, call `logFn('state-snapshot', { turn, phase, gold, tier })` — 2 tests: state-snapshot entry appears in logFn output on turn increment, no snapshot logged when turn unchanged — `apps/overlay/src/coordinator.ts` + `apps/overlay/src/coordinator.test.ts`
