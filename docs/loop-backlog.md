...
- [ ] [S] `scoreSellCandidate` in advisor: `packages/advisor/src/simScorer.ts` — add `scoreSellCandidate(projectedBoard: Board, player: PlayerState, opponents: OpponentState[], n: number): ScoreResult` which calls `scoreCandidate` (existing function); 2 tests: returns ScoreResult shape, n=0 returns zeroed result — `packages/advisor/src/simScorer.ts` update + test
