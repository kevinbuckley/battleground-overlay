...
- [ ] [S] `pipeline` reset test: in `packages/state/src/pipeline.test.ts`, add a test that applies 2 TAG_CHANGE events (incrementing turn), calls `pipeline.reset()`, then verifies `getState()` returns initial state (`turn === 0`, `phase === 'lobby'`); 2 tests: reset after events returns initial state, getState after reset has turn=0 — `packages/state/src/pipeline.test.ts` update
...
- [ ] [S] `simulateBatch` determinism test: in `packages/sim/src/simulateBatch.test.ts`, add a test calling `simulateBatch(playerBoard, oppBoard, 20, 42)` twice with the same seed and asserting the results are strictly equal (`wins`, `losses`, `ties` all match) — `packages/sim/src/simulateBatch.test.ts` update
- [ ] [S] `advisorDiff` in replay app: in `apps/replay/src/advisorDiff.ts`, verify `diffRecs(actual, expected)` returns an array of diff entries where each entry has `action`, `actualScore`, and `expectedScore`; add 3 tests: identical recs return no diffs, different action types return 1 diff, different scores return 1 diff with correct delta — `apps/replay/src/advisorDiff.test.ts` update
