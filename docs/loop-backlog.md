...
- [ ] [S] `waitForLogFile` export — export `waitForLogFile` from `packages/log-parser/src/index.ts`; 1 test in `packages/log-parser/src/index.test.ts`: `import { waitForLogFile } from '../src/index'` is a function — `packages/log-parser/src/index.ts` + `packages/log-parser/src/index.test.ts`
...
- [ ] [S] `isHearthstoneRunning` helper — add to `apps/overlay/src/anchor.ts` an exported function `isHearthstoneRunning(execFn: (cmd: string) => string = (cmd) => execSync(cmd, { encoding: 'utf-8' }).toString()): boolean` that runs `pgrep -x Hearthstone` and returns true if non-empty stdout; 3 tests in `anchor.test.ts`: with `execFn = () => '1234\n'` returns true; with `execFn = () => ''` returns false; with `execFn = () => { throw new Error('not found'); }` returns false — `apps/overlay/src/anchor.ts` + `apps/overlay/src/anchor.test.ts`
