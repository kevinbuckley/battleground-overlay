...
- [ ] [S] `getConfidenceLabel(c)` — add to `apps/overlay/src/renderer.ts` an exported function `getConfidenceLabel(c: number): 'high'|'medium'|'low'` (`>=0.7` → high, `>=0.4` → medium, else low); 4 tests: `0.9` → high, `0.7` → high, `0.5` → medium, `0.2` → low — `apps/overlay/src/renderer.ts` + `apps/overlay/src/renderer.test.ts`
