...
- [ ] [S] `shopMinionCount` helper: in `packages/shared/src/utils.ts`, add `shopMinionCount(state: GameState): number` returning `state.player.shop.minions.length`; export it; 2 tests: empty shop=0, 3 shop minions=3 — `packages/shared/src/utils.ts` update + test
